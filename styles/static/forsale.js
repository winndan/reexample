...
function getFavorites() {
    const favorites = localStorage.getItem('saleFavorites');
...
function saveFavorite(propertyId) {
    ...
    if (!favorites.includes(propertyId)) {
        ...
        localStorage.setItem('saleFavorites', JSON.stringify(favorites));
    }
}
...
function filterProperties() {
    ...
    cards.forEach(card => {
        ...
        // Location filter — match if card location contains search text
        ...
        // Bedrooms filter — each checked value means "at least this many"
        if (bedrooms.length > 0) {
            const bedNum = parseInt(cardBeds) || 0;
            const minBed = Math.min(...bedrooms.map(b => b === '5+' ? 5 : parseInt(b) || 0));
            if (bedNum < minBed) show = false;
        }
