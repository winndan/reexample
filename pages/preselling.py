...
def presellingpage(page=1):
    listings, total = get_paginated_listings("pre_selling", page=page, per_page=PER_PAGE)
    ...
    Head(
        *GoogleAnalytics("G-6NGE48T3KE"),
        ...
        Title('Pre-Selling Properties - Luxe Properties'),
        Meta(name='description', content='Explore pre-selling properties and upcoming developments. Invest early at the best prices.'),
        Link(rel='preconnect', href='https://fonts.googleapis.com'),
        Link(rel='preconnect', href='https://fonts.gstatic.com', crossorigin=''),
        Link(href='https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap', rel='stylesheet'),
        Link(rel='stylesheet', href='styles/components/navbar.css'),
        Link(rel='stylesheet', href='styles/components/footer.css'),
        Link(rel='stylesheet', href='styles/static/preselling.css')
    ),
    Body(
        Navbar(active="preselling"),
        Section(
            Div(
                Div(
                    A('Home', href='/'),
                ...
                H1('Pre-Selling Properties', cls='page-title'),
                P('Invest early in upcoming developments at the best pre-selling prices', cls='page-subtitle'),
                cls='container'
            ),
            cls='page-header'
        ),
            ...
            Div(
                Aside(
                    ...
                    Form(
                        ...
                        Div(
                            Label('Developer', cls='filter-label'),
                            Select(
                                Option('All Developers', value=''),
                                Option('Ayala Land', value='ayala'),
                                Option('SM Development', value='smdc'),
                                Option('Megaworld', value='megaworld'),
                                Option('DMCI Homes', value='dmci'),
                                Option('Robinsons Land', value='robinsons'),
                                id='developer', cls='filter-select'
                            ),
                            cls='filter-group'
                        ),
                        Div(
                            Label('Turnover Year', cls='filter-label'),
                            Select(
                                Option('Any Year', value=''),
                                Option('2026', value='2026'),
                                Option('2027', value='2027'),
                                Option('2028', value='2028'),
                                Option('2029+', value='2029'),
                                id='turnoverYear', cls='filter-select'
                            ),
                            cls='filter-group'
                        ),
                        Div(
                ...
                Div(
                    Div(
                        ...
                        Div(
                            ...
                            Select(
                                ...
                                Option('Turnover: Earliest', value='turnover'),
                    ...
                    Div(
                        *[_presell_card(str(l.get('id','')), l.get('title',''), l.get('price',''), l.get('price_label','Starting Price'),
                                        l.get('description',''), l.get('location',''), l.get('beds',''), l.get('baths',''),
                                        l.get('area',''), l.get('img_url',''), l.get('developer',''), l.get('turnover','')) for l in listings],
                        id='propertyGrid', cls='property-grid'
                    ),
                    _pagination_nav(page, total_pages, '/pre-selling'),
                    cls='listings-main'
                ),
        ...
        Script(src='styles/static/preselling.js', defer=True),
...
def _presell_card(data_id, title, price, price_label, description, location, beds, baths, area, img_url, developer, turnover):
    return Article(
        Div(
            Img(src=img_url, alt=title, loading='lazy', cls='property-image'),
            Div(
                Span('Pre-Selling', cls='badge badge-pre'),
                Span(f'Turnover {turnover}', cls='badge badge-turnover'),
                cls='property-badges'
            ),
            Div(
                Div(
        ...
        Div(
            Div(Span(developer, cls='developer-tag'), cls='developer-info'),
        ...
        data_id=data_id, cls='property-card'
    )
